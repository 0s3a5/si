...
app.get('/wallet', authMiddleware, async (req, res) => {
  ...
  const lista = await sql("SELECT * FROM boleta WHERE id_asignado = $1", [userId]);
  ...
  try {
      ...
      for (const producto of productosCarrito) {
          ...
          if (productoEnCarrito.length > 0) {
              ...
              // Actualizar el stock en la tabla producto o anime (según el caso)
              await sql(
                  `UPDATE producto SET cantidad = cantidad - $1 WHERE id_producto = $2`,
                  [cantidadComprada, idProducto]
              );
              await sql(
                  `UPDATE anime SET cantidad = cantidad - $1 WHERE id_producto = $2`,
                  [cantidadComprada, idProducto]
              );
          }
      }
